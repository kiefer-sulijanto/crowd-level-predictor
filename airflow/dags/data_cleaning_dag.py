...
def inline_feature_engineering():
    ...
    df_features = pd.DataFrame(
        df_clean.apply(lambda x: build_feature_dict(x.to_dict()), axis=1).tolist()
    )
...
with DAG(
    ...
    task_clean_data = PythonOperator(
        task_id="step_1_clean_raw_data", python_callable=clean_data_pipeline
    )

    # Task 2: Prep for ML (Cyclical time, holiday flags, etc.)
    task_engineer_features = PythonOperator(
        task_id="step_2_build_ml_features", python_callable=inline_feature_engineering
    )
